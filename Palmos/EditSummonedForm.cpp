/*
 * Source code for the Edit Summoned Creature Form.
 */
...
// Initiative modifier to display for new summoned
#define DEFAULT_MODIFIER 0
// Spell counter to display for new summoned
#define DEFAULT_COUNTER 1
// Default spell duration to display for new summoned
...
void Summoned::popupEditForm(void) {
	summoned = this;
	owner = getOwner();
	FrmPopupForm(EditSummFormID);
}

/* Pop up form for editing character. If argument is NULL, create a charcter. */
void FrmPopupEditSummForm(Effect* e) {
	summoned = NULL;
	owner = e;
	FrmPopupForm(EditSummFormID);
}
 ...
 */
Boolean EventHandlerEditSummForm(EventPtr event)
{
